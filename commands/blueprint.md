...
Output file: `docs/blueprint/<slug>.md`.
...
3. Render with Mermaid CLI via npx:
   ```
...
Take the agent's returned markdown body verbatim except: replace each `[[DIAGRAM:n]]` token with `![Diagram n](<slug>-n.png)`. Do not include any `<diagram>` tag or Mermaid source in the final `.md`.
