...
3. Resolve which service(s) the user means against the **intersection** of both lists:
   ...
   - **In `env.md` but not in `docker/docker-compose.yml`** → do not start anything; say it's documented but not yet defined in compose, and point to `/dev` to generate its infra spec
...
- After editing, print a short diff summary of what changed
- If the new/changed service has no matching entry in `docker/docker-compose.yml`, note it: "⚠️ not yet defined in docker/docker-compose.yml — run `/dev` to auto-generate its infra spec." Do not touch `docker/docker-compose.yml` yourself for a plain edit request — that belongs to the infra spec flow (the `infra` agent owns compose changes). Direct compose changes are only made here as part of **Starting/Opening Services**, and only to bring up what's already defined.
